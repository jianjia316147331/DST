...
def cmd_gen_result_msg():
    ...
    if company:
        try:
            from .db import _get_db_conn
            conn = _get_db_conn()
            # Count all violations for this company
            cur = conn.execute("""
                SELECT v.query_date, v.first_collection_time, v.handling_status_label,
                       v.fine_amount, v.points, v.plate_number
                FROM violations v
                JOIN vehicles veh ON v.vehicle_id = veh.id
                JOIN companies c ON veh.company_id = c.id
                WHERE c.name = ?
                ORDER BY v.query_date DESC, v.first_collection_time DESC
            """, (company,))
            rows = cur.fetchall()
            conn.close()

            if rows:
                today = p.get("date", "") or time.strftime("%Y-%m-%d")
                # Count violations first collected today (new this run)
