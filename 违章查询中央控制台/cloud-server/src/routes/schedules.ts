...
export default async function scheduleRoutes(app: FastifyInstance) {
  ...
  app.put('/api/schedules/:id', { preHandler: [app.authenticate] }, async (request, reply) => {
    ...
    if (cron_expression) {
      try {
        ...
        sets.push(`cron_expression = $${idx++}`); params.push(cron_expression);
    ...
    if (rows.length === 0) return reply.status(404).send({ error: 'Schedule not found' });
    return rows[0];
  });
  ...
  app.delete('/api/schedules/:id', { preHandler: [app.authenticate] }, async (request, reply) => {
    ...
    if (rowCount === 0) return reply.status(404).send({ error: 'Schedule not found' });
    return { success: true };
  });
}
